...
define({
	...
	loader: {
		...
		packages: [
			...
			{ name: 'dojoquest', location: 'src/dojoquest' }
		]
	},

	// Non-functional test suite(s) to run in each browser
	suites: [ 'dojoquest/tests/unit' ],

	// Functional test suite(s) to run in each browser once non-functional tests are completed
	functionalSuites: [ 'dojoquest/tests/functional' ],

	// A regular expression matching URLs to files that should not be included in code coverage analysis
	excludeInstrumentation: /^(?:node_modules|src\/(?:dojo|dijit|dojox|put-selector|xstyle|dgrid|dojoquest\/tests))\//
});
